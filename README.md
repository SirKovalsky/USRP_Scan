...
Есть масштабирование графика (рамка, кнопки, «К пику», подсказка под
курсором), устойчивая ось частот, пауза без прерывания приёма и запуск
на Windows и Linux.
...
- **Единый выбор единиц** (ГГц / МГц / кГц / Гц) для полей ввода, осей
  графиков, подписей пиков и таблицы. Деления оси подписаны **ровно в
  выбранной единице** (авто-приставки СИ отключены, поэтому «1 kМГц» вместо
  1000 МГц не появится).
- **Масштаб графика**: кнопки **「＋ Увеличить」/「－ Уменьшить」**, выделение
  ...
  возвращает обзор.
- **Устойчивая ось частот**: масштаб по X привязан к настроенному диапазону
  (Начало…Конец в свипе, центр ± полоса в реальном времени), поэтому ось **не
  проматывается** по мере поступления данных. Ручной зум мышью автоматически
  выключает автомасштаб и держится, пока не нажат **«Сброс вида»**.
- **Курсор-подсказка**: при наведении мыши на спектр показываются частота и
  уровень ближайшего бина — удобно «навестись и прочитать».
...
| Прочитать значение | навести мышь на спектр — рядом с курсором появятся частота и уровень ближайшего бина (пунктирная линия-«прицел») |
| Вернуть обзор | **«Сброс вида»** (снимает ручной зум спектра и водопада) |
...
**Ось частот не проматывается.** Масштаб по X берётся из настроек: в свипе
это «Начало…Конец», в реальном времени и ГНСС — «центр ± полоса приёма».
Данные свипа приходят постепенно, но ось стоит на месте — график не мельтешит,
и по нему можно спокойно наводиться. Если диапазон в настройках изменён
(в том числе пресетом), ось обновится сразу.

**Ручной зум — приоритетнее автомасштаба.** Как только вы приблизили график
рамкой или колесом, галочка «Авто-масштаб по X» (и по Y) снимается сама, и
вид остаётся таким, каким вы его сделали: новые кадры его больше не сбрасывают.
Вернуть автомасштаб можно галочкой или кнопкой «Сброс вида».

Зум ограничен физической полосой B210 (70 МГц … 6 ГГц) и настроенным
диапазоном: «уехать» в пустоту или выйти за пределы приёма нельзя.
...
- **Масштаб и «К пику».** Ручной зум меняет только вид (`ViewBox`), не
  трогая приём. Пределы осей заданы полосой B210, а «К пику» берёт окно
  шириной `max(2 × измеренная полоса, 20 × бин)` вокруг центра выбранного
  пика и передаёт те же границы водопаду (`WaterfallWidget.set_x_span`).
- **Стабильная ось X.** Приложение хранит «целевой» диапазон (`_display_span_hz`
  → `_apply_display_span`) и вызывает `setXRange` только когда он реально
  изменился (порог 0.5 %), поэтому приход кадров свипа больше не проматывает
  ось. Водопад получает тот же диапазон отдельным методом `set_auto_span` и
  тоже не следует за «растущими» данными. Ручной сдвиг/зум мышью ловится
  сигналом `sigRangeChangedManually`: он выключает галочки авто-масштаба и
  фиксирует текущий диапазон (у водопада — флаг `_view_locked`), поэтому
  выбранный вид держится кадр за кадром до нажатия «Сброс вида».
- **Ряды водопада при свипе.** Строка свипа с каждым кадром длиннее; буфер
  расширяется с сохранением ранее накопленных строк (они выровнены по началу
  диапазона), а при укорочении (начался новый свип) обнуляется.
...
| Ничего не видно после зума | Нажмите «Сброс вида» (или включите «Авто-масштаб по X»): ручной зум ограничен полосой B210 и настроенным диапазоном. |
| Ось частот «уезжала» при свипе | Исправлено: ось привязана к «Начало…Конец», а не к уже принятым данным. Если вы сами приблизили график, вид держится до «Сброса вида». |
| Хочу прочитать частоту под курсором | Наведите мышь на спектр — появится пунктирная линия-«прицел» с частотой и уровнем ближайшего бина. |
| На оси подписи вида «1 kМГц» | Исправлено: авто-приставки СИ отключены, деления подписываются в выбранной единице (1000 МГц так и пишется). |
